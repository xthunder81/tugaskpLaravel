...
<header id="header" class="fixed-top">
    ...
    <section id="hero" class="d-flex align-items-center" style="height: auto !important">
        <div class="container d-flex flex-column align-items-center justify-content-center" data-aos="fade-up">
            <h1 style="width: 100% !important">Blog {{ env("NAMASEKOLAH","") }}</h1>
            <h2 class="mb-4">Segala informasi mengenai kegiatan belajar mengajar akan diperbarui disini</h2>
        </div>
     ...
     <section id="syarat-pendaftaran" class="services">
        <div class="container" data-aos="fade-up">
                ...
                @foreach($blog as $b)
                    <a href="{{ route('siswa.blog.show', $b->id_blog) }}" style="color: #2d405f;">
                        <div class="row">
                            <div class="col-md-12 col-lg-12 col-sm-12 d-flex align-items-stretch mb-3 mb-lg-3" data-aos="fade-up" data-aos-delay="100">
                                <div class="icon-box col-md-12 col-lg-12 col-sm-12">
                                    ...
                                    <p class="description">Diunggah {{ \Carbon\Carbon::parse($b->created_at)->diffForHumans() }}</p>
                                </div>
                            </div>
                        </div>
                    </a>
            ...
            <div class="row" data-aos="fade-up" data-aos-delay="100">
                @if ($blog->lastPage() >= 1)
                    <ul class="pagination mx-auto">
                        <li class="page-item {{ ($blog->currentPage() == 1) ? ' disabled' : '' }}">
                            <a class="page-link" href="{{ $blog->url($blog->currentPage()-1) }}">&larr; Sebelumnya</a>
                        </li>
                        <li class="page-item {{ ($blog->currentPage() == $blog->lastPage()) ? ' disabled' : '' }}">
                            <a class="page-link" href="{{ $blog->url($blog->currentPage()+1) }}">Setelahnya &rarr;</a>
                        </li>
                    </ul>
                @endif
